...
public class Doc_AllocationHdr extends Doc
{
	...
	public ArrayList<Fact> createFacts (MAcctSchema as)
	{
		...
		for (int i = 0; i < p_lines.length; i++)
		{
			...
			if(line.getC_CashLine_ID() != 0)
				mcl = new MCashLine(getCtx(),line.getC_CashLine_ID(), null);
			...
			else
			{
				...
				if (as.isAccrual())
				{
					// Advance (Vladimir Sokolov)
					if(mcl != null){
						if(CashTypeAdvance.equals(mcl.getCashType())){
							bpAcct = getAccount(Doc.ACCTTYPE_CashTransferAdvance, as);
							if(Math.signum(allocationSource.doubleValue()) == -1.)
				...
				{
					BigDecimal bd = line.getAmtSource();
					// Advance (Vladimir Sokolov)
					if(mcl != null){
						if(CashTypeAdvance.equals(mcl.getCashType())){
							if(Math.signum(bd.doubleValue()) == 1.)
								bd = bd.negate();
						}
					}// Advance
					fl = fact.createLine (line, getCashAcct(as, line.getC_CashLine_ID()),
						getC_Currency_ID(), null, bd.negate());
					MCashLine cashLine = new MCashLine (getCtx(), line.getC_CashLine_ID(), getTrxName());
					if (fl != null && cashLine.get_ID() != 0)
						fl.setAD_Org_ID(cashLine.getAD_Org_ID());
				}
	...
	private MAccount getCashAcct (MAcctSchema as, int C_CashLine_ID)
	{
		...
		// Advance (Vladimir Sokolov)
		if(C_CashLine_ID != 0){
			MCashLine mc = new MCashLine(getCtx(),C_CashLine_ID, null);
			if(CashTypeAdvance.equals(mc.getCashType())){
				return getAccount(Doc.ACCTTYPE_CashTransferAdvance, as);
		}
	}

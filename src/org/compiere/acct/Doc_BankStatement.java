...
public class Doc_BankStatement extends Doc
{
	...
	public ArrayList<Fact> createFacts (MAcctSchema as)
	{
		...
		for (int i = 0; i < p_lines.length; i++)
		{
			...
			//a.nurpiisov 04032012:1109-------------------------------{
			MAccount acct_bank_asset = null;
			MAccount acct_bank_in_transit = null;
			setC_BPartner_ID(C_BPartner_ID);
			
			//Doc_Payment payment = new Doc_Payment(ass, rs, trxName);
			boolean st = false; 
			...
			if (st)
				acct_bank_in_transit = getAccount(Doc.ACCTTYPE_BankInTransit, as);
			
			acct_bank_asset =  getAccount(Doc.ACCTTYPE_BankAsset, as);
			//}--------------------------------------------------------

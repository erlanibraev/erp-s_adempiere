...
public class ProjectPhaseGenOrder  extends SvrProcess
{
	...
	protected String doIt() throws Exception
	{
		...
		MOrder order = new MOrder (fromProject,fromPhase,true, MOrder.DocSubTypeSO_OnCredit);
		order.setDateOrdered(fromPhase.getStartDate());
		order.setDatePromised(fromPhase.getEndDate());
		order.setDescription(order.getDescription() + " - " + fromPhase.getName());
		if (!order.save())
			throw new Exception("Could not create Order");
	    /*MOrderLine oline = new MOrderLine (order);
	    oline.setPriceAmt(fromPhase.getPlannedAmt());
	    oline.getPriceAmt();
	    StringBuffer sb1 = new StringBuffer (fromPhase.getName());
		if (fromPhase.getDescription() != null && fromPhase.getDescription().length() > 0)
			sb1.append(" - ").append(fromPhase.getDescription());
		oline.setDescription(sb1.toString());*/
		MOrderLine ol1 = new MOrderLine(order);
		ol1.setLine(fromPhase.getSeqNo());
		StringBuffer sb1 = new StringBuffer (fromPhase.getName());
		if (fromPhase.getDescription() != null && fromPhase.getDescription().length() > 0)
			sb1.append(" - ").append(fromPhase.getDescription());
		...
		ol1.setQty(fromPhase.getQty());
		ol1.setPrice(fromPhase.getPlannedAmt());

...
public class GridTab implements DataStatusListener, Evaluatee, Serializable
{
	...
	public String getTrxInfo()
	{
		...
		//Nurpiisov 23112011{
		if (m_vo.TableName.startsWith("C_Project"))
		{
			//JOptionPane.showMessageDialog(null, "getTrxInfo: " + m_vo.TableName.toString());
			MessageFormat mf = null;	
			Object arg [] = new Object[2];// "test";
//			arg[0] = "test";
//			arg[1] = "55";
			String sql = new String ("Select Count(*) as lines, Sum(invoicedamt) as Sum from c_project");
			boolean filled = false;
			PreparedStatement pstmt = null;
			ResultSet rs = null;
			try
			...
			if(filled){
				try
				{
				    mf = new MessageFormat(Msg.getMsg(Env.getAD_Language(m_vo.ctx), "ProjectSumm"), Env.getLanguage(m_vo.ctx).getLocale());
				}
				catch (Exception e)
				{
				      log.log(Level.SEVERE, "Nurpiisov: some error");//Level.SEVERE, "OrderSummary=" + Msg.getMsg(Env.getAD_Language(m_vo.ctx), "OrderSummaryWLeasedPrinter"), e);
				}

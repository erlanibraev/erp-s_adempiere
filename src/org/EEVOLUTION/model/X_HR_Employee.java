...
public class X_HR_Employee extends PO implements I_HR_Employee, I_Persistent 
{
	...
	/**	Static Logger	*/
	private static CLogger	s_log	= CLogger.getCLogger (X_HR_Employee.class);
	...
	protected boolean saveUpdate() {
	 ...
	 /** Load Model for BPartner */
    public static X_HR_Employee getHR_Employee (int C_BPartner_ID)
    {
    	if(C_BPartner_ID == 0)
    		return null;
    	
    	X_HR_Employee hremp = new X_HR_Employee(Env.getCtx(), 0, null);
    	PreparedStatement pstmt = null;
		ResultSet rs = null;
		
		// Employee data from the card
				String sql_ = "SELECT * FROM HR_EMPLOYEE WHERE HR_EMPLOYEE.C_BPARTNER_ID=? " +
						"AND HR_EMPLOYEE.AD_CLIENT_ID=? " +
						"AND HR_EMPLOYEE.ISACTIVE='Y'";
		try
		{
			...
			if(rs.next())
				hremp = new X_HR_Employee(Env.getCtx(), rs, null);			
		}
